...
@section('title', 'Edit Announcement - GBIS Mojoagung')
...
<div class="recent-section" style="max-width: 850px; margin: 0 auto; padding: 3rem; background: white; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.04);">
    <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST">
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <input type="text" name="title" value="{{ old('title', $announcement->title) }}" placeholder="Contoh: Jadwal Ibadah Minggu Ini"
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <textarea name="content" rows="6" placeholder="Tuliskan detail pengumuman di sini..."
                      style="width: 100%; padding: 1rem; border: 1px solid #e0e0e0; border-radius: 12px; font-family: inherit; transition: border-color 0.3s; resize: vertical;" required>{{ old('content', $announcement->content) }}</textarea>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 2rem; margin-bottom: 2.5rem;">
            <div>
                ...
                <select name="type" style="width: 100%; padding: 1rem; border: 1px solid #e0e0e0; border-radius: 12px; font-family: inherit; background: white;" required>
                    <option value="info" {{ old('type', $announcement->type) == 'info' ? 'selected' : '' }}>Info (Biru - Standar)</option>
                    <option value="success" {{ old('type', $announcement->type) == 'success' ? 'selected' : '' }}>Success (Hijau - Berhasil)</option>
                    <option value="warning" {{ old('type', $announcement->type) == 'warning' ? 'selected' : '' }}>Warning (Kuning - Perhatian)</option>
                    <option value="important" {{ old('type', $announcement->type) == 'important' ? 'selected' : '' }}>Important (Merah - Mendesak)</option>
                </select>
            </div>
            <div>
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 2.5rem; margin-bottom: 3rem; padding: 1.5rem; background: #f9f9f9; border-radius: 12px;">
            <label style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer; user-select: none;">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $announcement->is_active) ? 'checked' : '' }} style="width: 1.2rem; height: 1.2rem; cursor: pointer;"> 
                <span style="font-weight: 600; font-size: 1rem;">Aktifkan Pengumuman</span>
            </label>
            <label style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer; user-select: none;">
                <input type="checkbox" name="is_pinned" value="1" {{ old('is_pinned', $announcement->is_pinned) ? 'checked' : '' }} style="width: 1.2rem; height: 1.2rem; cursor: pointer;"> 
        ...
        <div class="form-actions" style="display: flex; gap: 1.5rem; justify-content: flex-end; padding-top: 2rem; border-top: 1px solid #f0f0f0;">
            ...
            <button type="submit" class="btn btn-primary" style="padding: 1rem 2.5rem; box-shadow: 0 5px 15px rgba(0,74,173,0.3);">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection
